...
const ProviderDashboard = () => {
  ...
  // Profile image — tracked in state so UI updates immediately on upload
  ...
  // Client-side service type filter
  ...
  const deleteBooking = async (id) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Cannot delete — must be COMPLETED");
    }
  };
  ...
  const addService = async (e) => {
    ...
    try {
      await API.post("/api/services", { ...newService, price: parseFloat(newService.price) });
      toast.success("Service created"); setNewService({ name: "", price: "", description: "" });
      fetchServices();
    ...
    switch (activeTab) {
            ...
            {filteredRequests.map((req) => {
              ...
              return (
                <div key={req.id} className="card">
                  ...
                  <p><strong>Price:</strong> ₹ {req.service?.price}</p>
                  {req.bookingLocation
      ...
      case "services": return (
        <div className="dashboard-section">
          ...
          <div style={{ background: "white", padding: 28, borderRadius: "var(--radius)", border: "1px solid var(--border)", marginBottom: 32, maxWidth: 520 }}>
          ...
          <ViewContainer mode={servicesView}>
            {services.map((s) => (
              <div key={s.id} className="card">
                ...
                <h3>{s.name}</h3>
                <p>{s.description}</p>
                <p style={{ fontSize: 18, fontWeight: 700, marginTop: 8 }}>₹ {s.price}</p>
  ...
  return (
    <div className="dashboard-layout">
      <aside className="sidebar">
        ...
        <NavButton tab="requests" label="Customer Requests" />
        <NavButton tab="services" label="My Services" />
        <NavButton tab="profile" label="My Profile" />
        <NavButton tab="reviews" label="Reviews" />
